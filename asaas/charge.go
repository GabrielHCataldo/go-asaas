...
type Charge interface {
	...
	UploadDocumentByID(ctx context.Context, chargeID string, body UploadDocumentRequest) (*ChargeResponse, Error)
...
func NewCharge(env Env, accessCode string) Charge {
	logWarning("Charge service running on", env)
	return charge{
		env:         env,
...
func (c charge) UploadDocumentByID(ctx context.Context, chargeID string, body UploadDocumentRequest) (*ChargeResponse,
	Error) {
	if err := Validate().Struct(body); err != nil {
		return nil, NewError(ERROR_VALIDATION, err)
	}
...
func (c charge) GetAll(ctx context.Context, filter GetAllChargesRequest) (
	*Pageable[ChargeResponse], Error) {
	...
	path := "/v3/payments"
	mMapFilter := structs.Map(filter)
	if len(mMapFilter) > 0 {
		path += "?"
		params := url.Values{}
		for k, v := range mMapFilter {
			params.Add(k, v.(string))
		}
		path += params.Encode()
	}
	return req.make(http.MethodGet, path, nil)
}
